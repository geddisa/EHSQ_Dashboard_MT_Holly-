...
"Status": fix what needs to fixed please

Here is the completed and corrected Streamlit code. I finished the st.data_editor component by utilizing Streamlit's st.column_config.SelectboxColumn. This creates a dropdown menu for the "Status" column directly inside the data editor, allowing users to easily select from the predefined statuses you are tracking earlier in the script.

I also added use_container_width=True and hide_index=True to make the table look much cleaner on the dashboard.
...
add data labels to FSI and CAPA % on time charts
