...
with tabs[0]: # Overview: Incidents by Type and Department
    ...
    # Categorization by Department (formerly Location)
...
with tabs[2]: # Housekeeping
    ...
    hk_data = df.groupby(['Department', 'Housekeeping_Status']).size().reset_index(name='Count')
    fig_hk = px.bar(hk_data, x='Department', y='Count', color='Housekeeping_Status', 
                    barmode='group', title="Housekeeping Status Tracking by Department")
